...
def build_adj_mat(node_features, edge_index, device):
    n_nodes = node_features.shape[0]
    adj_mat = torch.zeros((n_nodes, n_nodes)).to(device)
    # Use advanced indexing to directly set all edges in one operation
    adj_mat[edge_index[0], edge_index[1]] = 1
    # Add identity matrix to ensure deg_i > 0 and stabilize training
    adj_mat += torch.eye(adj_mat.shape[0], device=device)
...
def build_adj_mat_hashmap(train_dataset, val_dataset, test_dataset, device):
    adjacency_matrices = {"train": [], "val": [], "test": []}

    for data in train_dataset:
        adj_mat = build_adj_mat(data.x, data.edge_index, device)
